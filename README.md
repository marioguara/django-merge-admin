...
- Changelist action **"Unisci selezionati…"** with a confirmation page that shows, for every candidate, **its own recorded data** — with the fields that differ between records highlighted — plus exactly what will be moved.
- The preview follows the ModelAdmin's own `fieldsets`, so internal fields stay out; override with `merge_preview_fields`.
- Mobile first: the confirmation page is a card per record, readable on a 320px screen.
